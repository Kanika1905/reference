...
export const AuthProvider = ({ children }) => {
  ...
  // New: Bypass login using only phone number (no OTP)
  const bypassPhoneLogin = async (phone) => {
    try {
      dispatch({ type: 'SET_LOADING', payload: true });

      // Create a minimal user object
      const user = { phone, isPhoneVerified: false };

      // Persist minimal user info; do NOT set real tokens
      await AsyncStorage.setItem(
        CONFIG.STORAGE_KEYS.USER_DATA,
        JSON.stringify(user)
      );

      // Mark as authenticated without tokens
      dispatch({
        type: 'LOGIN_SUCCESS',
      ...
      console.error('AuthContext - bypassPhoneLogin error:', error);
      return { success: false, message: 'Bypass login failed' };
    } finally {
      dispatch({ type: 'SET_LOADING', payload: false });
    }
  };
  ...
  const value = {
    ...
    bypassPhoneLogin,    // Bypass OTP and authenticate with phone only
    register,
    logout,
    updateUser,
    forgotPassword,
